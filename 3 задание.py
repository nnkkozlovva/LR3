# TODO  Напишите функцию count_letters


# TODO Напишите функцию calculate_frequency

def count_letters(text):
    """
    Подсчитывает количество каждой буквы в тексте.
    Буквы верхнего и нижнего регистра считаются одинаковыми.
    Возвращает словарь с буквами нижнего регистра и их количеством.
    """
    letter_count = {}

    for char in text:
        lower_char = char.lower()
        if lower_char.isalpha():
            letter_count[lower_char] = letter_count.get(lower_char, 0) + 1

    return letter_count


def calculate_frequency(letter_count_dict):
    """
    Вычисляет частоту каждой буквы.
    Принимает словарь с буквами и их количеством.
    Возвращает словарь с буквами и их частотами.
    """
    total_letters = sum(letter_count_dict.values())
    frequency_dict = {}

    for letter, count in letter_count_dict.items():
        frequency = count / total_letters
        frequency_dict[letter] = round(frequency, 2)  # Округляем до двух знаков

    return frequency_dict


# Текст для анализа
main_str = """
У лукоморья дуб зелёный;
Златая цепь на дубе том:
И днём и ночью кот учёный
Всё ходит по цепи кругом;
Идёт направо — песнь заводит,
Налево — сказку говорит.
Там чудеса: там леший бродит,
Русалка на ветвях сидит;
Там на неведомых дорожках
Следы невиданных зверей;
Избушка там на курьих ножках
Стоит без окон, без дверей;
Там лес и дол видений полны;
Там о заре прихлынут волны
На брег песчаный и пустой,
И тридцать витязей прекрасных
Чредой из вод выходят ясных,
И с ними дядька их морской;
Там королевич мимоходом
Пленяет грозного царя;
Там в облаках перед народом
Через леса, через моря
Колдун несёт богатыря;
В темнице там царевна тужит,
А бурый волк ей верно служит;
Там ступа с Бабою Ягой
Идёт, бредёт сама собой,
Там царь Кащей над златом чахнет;
Там русский дух… там Русью пахнет!
И там я был, и мёд я пил;
У моря видел дуб зелёный;
Под ним сидел, и кот учёный
Свои мне сказки говорил.
"""

# Подсчитываем буквы
letters_count = count_letters(main_str)

# Вычисляем частоты
letters_frequency = calculate_frequency(letters_count)

# Распечатываем в столбик букву и её частоту в тексте
# Сортируем по частоте (от большего к меньшему), как в примере
for letter, frequency in sorted(letters_frequency.items(), key=lambda x: x[1], reverse=True):
    print(f"{letter}: {frequency:.2f}")

# TODO Распечатайте в столбик букву и её частоту в тексте
